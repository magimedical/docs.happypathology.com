...
| `secure_file_urls` | `map[string]string` | A map of temporary URLs to download the files that were used to create this case. The URLs expire after 5 minute. |
| `medical_data` | `object` | Extracted medical documents — see [Medical Document](#medical-document-medical_data) |

:::caution
The `secure_file_urls` are dynamically generated every time `/v1/patient_case/$CASE_ID/extract` is called. The file URLs expire after 5 minute for security and compliance purposes. Call `/v1/patient_case/$CASE_ID/extract` again to receive fresh file URLs.
:::
